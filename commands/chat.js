                    ...
                    try {
                        ...
                        // 複数メッセージの場合
                            ...
                            for (let i = 0; i < splitMessages.length; i++) {
                                ...
                                if (i === 0) {
                                    await interaction.editReply({ content: `${messenger.answerFollowMessages(anthropicEmoji, message, i + 1, splitMessages.length)}\r\n`, ephemeral: !isPublic });
                                } else {
                                    await interaction.followUp({ content: `${messenger.answerFollowMessages(anthropicEmoji, message, i + 1, splitMessages.length)}\r\n`, ephemeral: !isPublic });
                                }
                            }
                        }

                        // 質問と回答をファイルに書き込む
                        await logger.answerToFile(interaction.user.id, request.trim(), attachmentContent.trim(), answer.text.trim());
                    } catch (error) {
                        // Discord の文字数制限の場合
